...
export default function Interviews() {
  ...
  // If we ever want to use feedback passed from another page
  useEffect(() => {
  ...
  const getModeDisplay = (mode) => {
    ...
    if (m.includes("behavioral") || m.includes("behavioural") || m.includes("hr"))
      return "HR / Behavioral";
    if (m.includes("coding") || m.includes("code") || m.includes("dsa"))
      return "Coding Challenge";
    if (m.includes("custom")) return "Custom Round";
    return mode;
  };
  ...
  return (
    <div className="space-y-8">
      {/* Gradient Header (Dashboard-style) */}
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-6 shadow-lg flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-purple-100">
            All your mock interview sessions in one place, with performance
            stats.
          </p>
        </div>
        <Button
      ...
      {/* Stats Row (matching dashboard cards) */}
      {interviews.length > 0 && (
      ...
      {/* Main Card – styled like Dashboard tables */}
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Review past sessions, scores and open detailed feedback.
            </p>
          </div>
        ...
        <div className="px-6 py-4">
          ...
          {/* Table view (similar to Dashboard Interview History) */}
          {!loading && !error && interviews.length > 0 && (
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40">
                    <th className="text-left px-6 py-3 text-[11px] font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                      Date
                    </th>
                    <th className="text-left px-6 py-3 text-[11px] font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                      Role
                  ...
                  {interviews.map((interview) => {
                    ...
                    const conf =
                      typeof interview.average_confidence === "number"
                        ? `${Math.round(
                            interview.average_confidence * 100
                          )}%`
                        : "--";
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-3 text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        ...
                        <td className="px-6 py-3">
                          <span className="inline-flex items-center px-2 py-1 rounded-md bg-purple-50 dark:bg-purple-900/40 text-[11px] font-medium text-purple-700 dark:text-purple-300">
                        ...
                        <td className="px-6 py-3 text-xs">
                          {primary ? (
                            <div
                              className={`inline-flex flex-col px-2.5 py-1 rounded-lg border text-left ${getScoreBgColor(
                                primary.value
                              )}`}
                            >
                              <span
      ...
      {!loading && !error && interviews.length > 0 && (
        <p className="text-[11px] text-gray-500 dark:text-gray-400 text-center">
          ...
          interview{interviews.length !== 1 && "s"}. Keep going to build your
          streak and unlock more badges on the dashboard.
        </p>
      )}
    </div>
  );
